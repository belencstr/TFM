...
def preparar(mapa):
    ...
    return matriz,float(exacta["coste_total"]),qubo

def ejecutar_instancia(nombre,mapa):
    matriz,optimo,qubo=preparar(mapa)

    resolver_qubo_simulated_annealing(
        qubo,num_reads=5,num_sweeps=5,seed=999999
    )
    ...
    for sweeps in SWEEPS_LIST:
        ...
        for seed in SEEDS:
            ...
            for muestra in resultado["muestras"]:
                a=muestra["asignacion"]
                occ=int(muestra["num_occurrences"])
                total += occ
        ...
        r={
            "instancia":nombre,
            "vars":qubo["numero_variables"],
            "sweeps":sweeps,
            "p_fact":p_fact,
            "p_opt":p_opt,
            "t_read":t_read,
            "tts_fact":tts(t_read,p_fact),
            "tts_opt":tts(t_read,p_opt),
        }
        resultados.append(r)

        print(
            f"{nombre} | vars={r['vars']} | sweeps={sweeps:4d} | "
            f"p_fact={100*p_fact:6.2f}% | p_opt={100*p_opt:6.2f}% | "
            f"t/read={t_read:.6f}s | TTS99_fact={ft(r['tts_fact'])}s | "
...
def ejecutar():
    print("="*120)
    print("CASO 1 — BARRIDO SA Y TTS99 — V2 CON WARM-UP")
    print("="*120)
    print(f"Seeds: {SEEDS}")
    print(f"Reads/seed: {NUM_READS}")
    ...
    for nombre,mapa in (
        ...
        todos.extend(ejecutar_instancia(nombre,mapa))

    print()
    print("MEJOR TTS99 AL OPTIMO")
    for nombre in ("A-8","B-9","C-10"):
        rs=[r for r in todos if r["instancia"]==nombre and not math.isinf(r["tts_opt"])]
        if not rs:
            print(f"{nombre}: no se observa optimo")
            continue
        m=min(rs,key=lambda r:r["tts_opt"])
        print(
            f"{nombre}: sweeps={m['sweeps']}, "
            f"p_opt={100*m['p_opt']:.2f}%, "
            f"TTS99={m['tts_opt']:.6f}s"
        )
...
if __name__=="__main__":
    ...
    ruta=os.path.join(carpeta,f"sa_tts_barrido_v2_{marca}.txt")
