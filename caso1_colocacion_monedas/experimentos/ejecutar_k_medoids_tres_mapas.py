"""Ejecuta k-medoids PAM sobre los mapas A, B y C.
...
class Tee:
    """Duplica stdout: consola + fichero TXT."""
...
def ejecutar_mapa(nombre, mapa):
    ...
    solucion = k_medoids_pam(candidatas, matriz, K)
    tiempo_solver = time.perf_counter() - inicio_solver

    print("=" * 72)
    print(f"CASO 1 — K-MEDOIDS PAM — MAPA {nombre}")
    print("=" * 72)
    ...
    print("Objetivo: minimizar la suma de distancias a la moneda más cercana")
    print()

    print("Medoides tras BUILD:")
    for candidata in solucion["candidatas_iniciales"]:
        print(
            f"  {candidata['id']} -> "
    ...
    print()
    print("Monedas seleccionadas tras SWAP:")
    ...
    print()
    print(f"Coste tras BUILD: {solucion['coste_build']}")
    print(f"Coste total final: {solucion['coste_total']}")
    print(
    ...
    print(f"Intercambios SWAP realizados: {solucion['intercambios']}")
    print(f"Tiempo matriz de distancias: {tiempo_matriz:.4f} s")
    print(f"Tiempo k-medoids PAM: {tiempo_solver:.4f} s")
    print()
    ...
    return {
        ...
        "coste": solucion["coste_total"],
        "dist_media": solucion["distancia_media_cobertura"],
...
def ejecutar_experimento():
    ...
    print("Algoritmo: k-medoids PAM (BUILD + SWAP)")
    print(f"k: {K}")
    print("Mapas evaluados: A, B y C")
    print()

    for nombre, mapa in (("A", MAPA_A), ("B", MAPA_B), ("C", MAPA_C)):
    ...
    print("=" * 88)
    print(
        f"{'Mapa':<7}{'Candidatas':>12}{'Coste':>10}"
        f"{'Dist. media':>14}{'Radio':>10}{'Sep. mín.':>12}{'Tiempo (s)':>14}"
    )
    for r in resultados:
        print(
            f"{r['mapa']:<7}{r['candidatas']:>12}{r['coste']:>10}"
            f"{r['dist_media']:>14.4f}{r['radio']:>10}"
            f"{r['sep_min']:>12}{r['tiempo']:>14.4f}"
        )
...
def main():
    ...
    nombre_fichero = f"k_medoids_pam_tres_mapas_k{K}_{marca_tiempo}.txt"
