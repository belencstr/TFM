...
def energia_qubo(Q,offset,muestra):
    ...
    for (v1,v2),coef in Q.items():
        energia += coef*muestra.get(v1,0)*muestra.get(v2,0)
    return energia
...
def evaluar_restricciones(grafo,muestra):
    ...
    for n in grafo:
        if n in (START,GOAL):
            continue
        diferencia=entradas[n]-salidas[n]
        if diferencia!=0:
            violaciones.append((n,entradas[n],salidas[n]))
        p_flujo += C*diferencia*diferencia

    n_saltos=len(activas)
    n_subidas=sum(1 for o,d in activas if d[1]>o[1])
    n_bajadas=sum(1 for o,d in activas if d[1]<o[1])
    n_planos=sum(1 for o,d in activas if d[1]==o[1])

    p_longitud=D*(n_saltos-L_OBJETIVO)**2
    p_subidas=E*(n_subidas-SUBIDAS_OBJETIVO)**2
    p_bajadas=F*(n_bajadas-BAJADAS_OBJETIVO)**2

    total=p_inicio+p_meta+p_flujo+p_longitud+p_subidas+p_bajadas

    return {
        "salida_start":salidas[START],
        "entrada_goal":entradas[GOAL],
        "numero_saltos":n_saltos,
        "numero_subidas":n_subidas,
        "numero_bajadas":n_bajadas,
        "numero_planos":n_planos,
        "violaciones_flujo":violaciones,
        "penalizacion_inicio":p_inicio,
        "penalizacion_meta":p_meta,
        "penalizacion_flujo":p_flujo,
        "penalizacion_longitud":p_longitud,
        "penalizacion_subidas":p_subidas,
        "penalizacion_bajadas":p_bajadas,
